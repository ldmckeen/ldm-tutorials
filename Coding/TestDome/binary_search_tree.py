...
Write a function that, efficiently with respect to time used, checks
...
def recursive_contains(root, value):
    if not root:
        return False
    if root.value == value:
        return True
    if value < root.value:
        return recursive_contains(root.left, value)
    
    return recursive_contains(root.right, value)


def iterative_contains(root, value):
    current = root
    while current:
...
if __name__ == '__main__':
    ...
    print(recursive_contains(n2, 3))
    print(iterative_contains(n2, 3))
